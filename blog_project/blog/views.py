...
def global_setting(request):
    ...
    #文章排行榜数据
    #评论排行
    comment_cout_list=Comment.objects.values('article').annotate(comment_count=Count('article')).order_by('-comment_count')
    article_comment_list= [Article.objects.get(pk=comment['article'])for comment in comment_cout_list]
    return locals()
    # return {
